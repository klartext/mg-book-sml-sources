{\rtf1\ansi\deff0\deftab720{\fonttbl{\f0\fnil MS Sans Serif;}{\f1\fnil\fcharset2 Symbol;}{\f2\fswiss\fprq2 System;}{\f3\fnil Times New Roman;}{\f4\fnil Courier Neu;}}
{\colortbl\red0\green0\blue0;}
\deflang1031\pard\plain\f4\fs20 (* Parallele Kombinatormaschine + Implementierung der PR*)\
\
fun last l = hd(rev l);\
fun remove x [] = []\
   |remove x (hd::tl) =\
\tab if x = hd then remove x tl\
        else hd::(remove x tl);\
\
(* ****************** *)\
...
\tab ENV := (name, alloc(ropt(c(read value))))::(!ENV);\
\
\
exception Lookup;\
fun lookup name =\
...
fun apply (WAIT,(node as ref(app((_,x),m,q)))::_) =\
   ...
   |apply (B,(ref(app((_,x),_,_)))::(ref(app((_,y),_,_)))\
              ::(node as (ref(app((_,z),m,q))))::_) =\
\tab node := app((x,ref (app((y,z),ref Eval,q))),ref Eval,q)\
   |apply (C,(ref(app((_,x),_,_)))::(ref(app((_,y),_,_)))\
              ::(node as (ref(app((_,z),m,q))))::_) =\
\tab node := app((ref(app((x,z),ref Eval,q)),y),ref Eval,q)\
\
